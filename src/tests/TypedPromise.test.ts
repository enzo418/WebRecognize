...
describe('Typed promise general tests', () => {
    ...
    it('should call fail', () => {
        //
        const promise = new TypedPromise<string, {title: string}>((ok, fail) =>
            fail({title: 'invalid name'}),
        );
    ...
    it('should call first catch if ok fails', (done) => {
        const promise = new TypedPromise<number, any>((ok, fail) => setTimeout(() => ok(1), 500));
    ...
    it('should catch an expection at some level and then call finally', (done) => {
        const promise = new TypedPromise<number, any>((ok, fail) => setTimeout(() => ok(418), 500));
