...
export default function ROICanvasInputField(props:ROICanvasInputFieldProps) {
    ...
    const [initialROI, setInitialROI] = useState<Rectangle>({x: 0, y:0, width:0, height:0});
    
    
    const getCameraPreview = () => {
        setLoading(true);

        const query = props.uri ? {uri: props.uri} : {camera_id: props.camera_id};

        client.get(config.endpoints.api.cameraFrame, query, {cache: "force-cache"})
            ...
            .then(buffer => {
                ...
                props.getFieldCB.apply(null, [`cameras/${props.camera_id}/${props.fieldPath}`])
                .ok((storedRoi) => {
                    ensure<Rectangle>(storedRoi);
                    
                    // we need the resize at the processing stage to scale down/up the selected ROI
                    props.getFieldCB.apply(null, [`cameras/${props.camera_id}/processingConfiguration/resize`])
                    .ok((resize:Size) => {
                        const scaleX = 640.0 / resize.width;
                        const scaleY = 360.0 / resize.height;
                        
                        setInitialROI(scaleRectangle(storedRoi, scaleX, scaleY));
                        setLoading(false);
                    }).fail(e => console.error("Could not get last resize (processing): ", e))
                })
                .fail((e) => {
    ...
    const onRoiUpdated = (roi: Rectangle) => {
        ...
        props.getFieldCB.apply(null, [`cameras/${props.camera_id}/processingConfiguration/resize`])
            .ok((resize:Size) => {
                ...
                let scaledRoi = scaleRectangle(roi, scaleX, scaleY);
                
                props.updateCB.apply(null, [`cameras/${props.camera_id}/${props.fieldPath}`, scaledRoi])
                    .ok(() => console.log("ROI Field updated"))
                    .fail(e => console.error("Could not update ROI", e))
            }).fail(e => console.error("Could not get last resize (processing): ", e))
    }

    return <>
        { loading ? <Skeleton variant="rectangular" width={640} height={360} /> 
            : <CanvasHandlerROI image={image} onRoiUpdated={onRoiUpdated} initialROI={initialROI}  />
        }
    </>
}