...
export default class CameraFrameBox extends React.Component<CameraFrameBoxProps, CameraFrameBoxState> {
    ...
    updateLiveFeed() {
        const query = this.props.uri ? {uri: this.props.uri} : {camera_id: this.props.camera_id};
        ...
        client.get(config.endpoints.api.cameraFrame, query, {cache: "force-cache"})
            ...
            .then(buffer => {
                ...
                this.setState({loading: false, error: e.title || "unknown error"});
    ...
    componentDidUpdate(prevProps:CameraFrameBoxProps) {
        ...
        if (this.props.uri !== prevProps.uri || this.props.camera_id !== prevProps.camera_id) {
            this.updateLiveFeed();
        }
    }
    ...
    render() {
        const imageStyle = Object.assign({
            display: (this.state.error.length !== 0 || this.state.loading) ? "none" : "block"
        }, this.props.style);
