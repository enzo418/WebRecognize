...
export default class ConfigurationServiceMock extends Service {
    ...
    public getField(id:string, path: string) {
        ...
        if (field === 'name') {
            return this.processPromise<any, IProblemJson>(Promise.resolve(new Response(
                getBlobFromObject({value: 'got this value from mock!'}), {
                    status: 200,
                    headers: {
    ...
    public setField(id:string, data: {field: string, value: any}) {
        ...
        if (field == 'url' && data.value.indexOf('418') !== 0) { // only sim error on name field
        ...
        return this.processPromise<any, IProblemJson>(promise);
    }
}
